package com.jenna.pennypilot.domain.category.controller;
...
public class CategoryController {
    ...
    @Operation(summary = "사용자의 카테고리(구분) 전체 조회", description = "userId 기준")
    ...
    @Operation(summary = "카테고리(구분) 정보 상세 조회", description = "하나의 카테고리 정보 by id")
    ...
    @Operation(summary = "카테고리(구분) 정보 추가")
    @PostMapping("")
    public ResponseEntity<?> addCategory(@RequestBody CategoryDTO category) {
        return ResponseEntity.ok(categoryService.addCategory(category));
    }

    @Operation(summary = "카테고리(구분) 정보 업데이트", description = "카테고리 id를 기준으로, 카테고리명 변경 가능")
    @PutMapping("")
